                  ...
                  stage('docker build'){                          
                     ...
                     steps{                            
                        script{  
                          ...
                          sh 'docker exec --tty $(docker ps -ql) sh -c "cat <<EOF > /etc/docker/daemon.json{"insecure-registries": ["ec2-52-39-183-6.us-west-2.compute.amazonaws.com:8123"]}"'           
                          ...
                          sh 'docker build -f Dockerfile -t ec2-52-39-183-6.us-west-2.compute.amazonaws.com:8123/scratch:5.0.0 .'                          
                          //sh 'docker restart $(docker ps -ql)'
                          //sh 'ls /etc/docker'     
                          sh 'docker login -u admin -p admin123 ec2-52-39-183-6.us-west-2.compute.amazonaws.com:8123'
                          sh 'docker push ec2-52-39-183-6.us-west-2.compute.amazonaws.com:8123/scratch:5.0.0'     
                               }           
