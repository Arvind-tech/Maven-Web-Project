                     ...
                     steps{                            
                        script{  
                         ...
                         // sh 'docker exec --tty $(docker ps -ql) sh -c "mkdir -p /root/.docker"'                                                       
                          ...
                          //sh 'docker run --rm -d --privileged docker:dind --names dazzling_gates'                               
                          //sh 'docker run --name dazzling_gates'                          
                          sh 'docker build -f Dockerfile -t ec2-52-36-87-109.us-west-2.compute.amazonaws.com:8123/scratch:6.0.0 .'                          
                          //sh 'docker restart $(docker ps -ql)'
                          //sh 'ls /etc/docker'     
                          sh 'docker login -u admin -p admin123 ec2-52-36-87-109.us-west-2.compute.amazonaws.com:8123'
                          sh 'docker push ec2-52-36-87-109.us-west-2.compute.amazonaws.com:8123/scratch:6.0.0'     
                               }           
